...
Partner Showcase uses a **simple, normalized schema** optimized for local storage with optional cloud sync capability.
...
function loadData(): LocalStorageSchema {
  const raw = localStorage.getItem('partner showcase-data');
